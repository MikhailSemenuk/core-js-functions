...
function getPowerFunction(exponent) {
  ...
  const powBind = pow.bind(null, exponent);
  return powBind;
}
...
function getPolynom(...args) {
  return function polynom(x) {
    const copyArgs = args.slice();
    return copyArgs.reverse().reduce((acc, value, index) => {
      return acc + value * x ** index;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return function innerLogger(...args) {
    ...
    const start = `${func.name}(${argsToString(args)}) starts`;
    const end = `${func.name}(${argsToString(args)}) ends`;
    logFunc(start);
...
function partialUsingArguments(fn, ...args1) {
  return function partialUsingArgumentsInner(...args2) {
    return fn.apply(undefined, [...args1, ...args2]);
  };
}
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function innerIdGenerator() {
